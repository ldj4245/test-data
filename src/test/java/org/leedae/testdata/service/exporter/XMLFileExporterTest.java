...
@DisplayName("[Logic] XML 파일 출력기 테스트")
@ExtendWith(MockitoExtension.class)
class XMLFileExporterTest {

    @InjectMocks private XMLFileExporter sut;
    @Mock private MockDataGeneratorContext mockDataGeneratorContext;

    @DisplayName("테이블 스키마 정보와 행 수가 주어지면, XML 형식의 문자열을 생성한다.")
    @Test
    void givenSchemaAndRowCount_whenExporting_thenReturnsXMLFormattedString(){
        // Given
        ExportFileType exportFileType = ExportFileType.XML;
        ...
        assertThat(result).contains("<data>");  // 루트 요소가 'data'
        assertThat(result).contains("<row>");
        assertThat(result).contains("<id>", "<age>", "<car>", "<created_at>");
        assertThat(result).endsWith("</data>");
